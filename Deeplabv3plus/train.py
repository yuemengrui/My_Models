...
class Trainer:
    def __init__(self, configs):
        ...
        self.metrics = {'OverallAcc': 0, 'MeanAcc': 0, 'FreqWAcc': 0, 'MeanIoU': 0, 'ClassIoU': 0, 'loss': float('inf'),
                        'best_model_epoch': 0}
    ...
    def _train_epoch(self, epoch):
        ...
        for i, (img, label) in enumerate(self.train_loader):
            ...
            logger.train.info(
                '[epoch:{}/{}], [iter:{}/{}], global_step:{}, loss:{}, lr:{:.9f}, reader_cost:{:.2f}s, batch_cost:{:.2f}s, speed:{:.1f}samples/sec,'.format(
    ...
    def _on_epoch_finish(self):
        ...
        if self.epoch_result['epoch'] % 20 == 0:
    ...
    def _save_checkpoint(self, epoch, path=None):
        ...
        :param save_best: if True, rename the saved checkpoint to 'model_best_V1.pth.tar'
        """
    ...
    def _initialize(self):
        start = time.time()
        self.model = Model(num_classes=self.configs.get('num_classes'))

        weight = torch.tensor([0.8, 1.5, 6.0, 1.0, 1.2, 1.2])
        ...
        self.metric_cls = SegMetrics(n_classes=self.configs.get('num_classes'))
        ...
        train_dataset = SegDataset(self.configs.get('dataset_dir'), 'train')
        val_dataset = SegDataset(self.configs.get('dataset_dir'), 'val')
...
if __name__ == '__main__':
    ...
    configs = {
        ...
        'dataset_dir': '/data/guorui/SegFormer/train_dataset',
        'Epochs': 500,
        'batch_size': 6,
        'num_classes': 5,
        'lr': 0.01,
